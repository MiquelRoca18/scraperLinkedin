...
def test_check_daily_budget_bajo_limite():
    ...
    try:
        with patch("main.get_daily_count", return_value=30):
...
def test_check_daily_budget_limite_alcanzado():
    ...
    main_module.MAX_CONTACTS_PER_DAY = 80
    try:
        with patch("main.get_daily_count", return_value=80):
...
def test_check_daily_budget_superado():
    ...
    main_module.MAX_CONTACTS_PER_DAY = 80
    try:
        with patch("main.get_daily_count", return_value=95):
            assert main_module._check_daily_budget("u") is True
    finally:
        main_module.MAX_CONTACTS_PER_DAY = orig
            ...
            with patch("main.get_username_non_interactive", return_value="test-user"):
                with patch("main.scrape_profile_and_connections", return_value=(perfil, conexiones)):
                    with patch("main.insert_run") as mock_insert:
                        with patch("main.update_account_last_run"):
                            orig_cwd = os.getcwd()
                            try:
                                os.chdir(tmp)
...
def test_run_index_encola_slugs_y_muestra_stats():
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        with patch("main.init_client", return_value=fake_account):
            with patch("main.get_username_non_interactive", return_value="yo"):
                with patch("main.collect_all_slugs", return_value=["a", "b", "c"]):
                    with patch("main.queue_slugs", return_value=3) as mock_queue:
                        with patch("main.get_queue_stats", return_value={
                            "pending": 3, "done": 0, "error": 0, "total": 3
                        }):
                            with patch("main._check_time_window", return_value=False):
                                with patch("main._check_daily_budget", return_value=False):
...
def test_run_index_sin_slugs_no_encola():
    with tempfile.TemporaryDirectory() as tmp:
        ...
        with patch("main.init_client", return_value=fake_account):
            with patch("main.get_username_non_interactive", return_value="yo"):
                with patch("main.collect_all_slugs", return_value=[]):
                    with patch("main.queue_slugs") as mock_queue:
                        with patch("main._check_time_window", return_value=False):
                            with patch("main._check_daily_budget", return_value=False):
                                main_module.run_index(interactive=False)
...
def test_run_enrich_procesa_slugs_pendientes_y_guarda_en_db():
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        with patch("main.init_client", return_value=fake_account):
            with patch("main.get_username_non_interactive", return_value="yo"):
                with patch("main._check_time_window", return_value=False):
                    with patch("main.get_daily_count", return_value=0):
                        with patch("main.get_pending_slugs", return_value=["alice"]):
                            with patch("main._create_driver_with_cookies", return_value=fake_driver):
                                with patch("main._enrich_connection_from_profile", return_value=fake_data):
                                    with patch("main.upsert_contact", return_value="inserted") as mock_upsert:
                                        with patch("main.mark_queue_done") as mock_done:
                                            with patch("main.insert_run"):
                                                with patch("main.get_queue_stats", return_value={
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        with patch("main.init_client", return_value=fake_account):
            with patch("main.get_username_non_interactive", return_value="yo"):
                with patch("main._check_time_window", return_value=False):
                    with patch("main.get_daily_count", return_value=0):
                        with patch("main.get_pending_slugs", return_value=[]):
                            with patch("main.get_queue_stats", return_value={
                                "pending": 0, "done": 10, "error": 0, "total": 10
                            }):
                                with patch("main._create_driver_with_cookies") as mock_driver:
                                    main_module.run_enrich(interactive=False)
...
def test_run_enrich_presupuesto_agotado_no_procesa():
    with tempfile.TemporaryDirectory() as tmp:
        ...
        with patch("main.init_client", return_value=fake_account):
            with patch("main.get_username_non_interactive", return_value="yo"):
                with patch("main._check_time_window", return_value=False):
                    with patch("main.get_daily_count", return_value=80):  # = límite
                        with patch("main._check_daily_budget", return_value=True):
                            with patch("main._run_safety_checks") as mock_checks:
    ...
    try:
        with patch("main.init_client", return_value=fake_account):
            with patch("main.get_username_non_interactive", return_value="yo"):
                with patch("main._check_time_window", return_value=False):
                    with patch("main.get_daily_count", return_value=0):
                        with patch("main.get_pending_slugs", return_value=["alice", "bob"]):
                            with patch("main._create_driver_with_cookies", return_value=fake_driver):
                                # alice existe y fue scrapeada hace 5 días (< 30) → skip
                                # bob no existe → debe visitarse
                                with patch("main.contact_exists", side_effect=[True, False]):
                                    with patch("main.days_since_last_scrape", return_value=5.0):
                                        with patch("main._enrich_connection_from_profile",
                                                   return_value={"profile_id": "bob"}) as mock_enrich:
                                            with patch("main.upsert_contact", return_value="inserted"):
                                                with patch("main.mark_queue_done") as mock_done:
                                                    with patch("main.mark_queue_error"):
                                                        with patch("main.insert_run"):
                                                            with patch("main.get_queue_stats",
...
def test_run_enrich_fuera_de_franja_lanza_en_no_interactivo():
    with tempfile.TemporaryDirectory() as tmp:
        ...
        with patch("main.init_client", return_value=fake_account):
            with patch("main.get_username_non_interactive", return_value="yo"):
                with patch("main.get_daily_count", return_value=0):
                    with patch("main._check_time_window", return_value=True):
